...
use zkemail_helpers::{
    generate_email_inputs, generate_email_with_regex_inputs, read_email_file, read_json_file,
    AbiDecodable,
};
...
async fn main() {
    ...
    if let Some(regex_config) = args.regex_config.as_ref() {
        ...
        let regex_info = read_json_file(regex_config).expect("Failed to read regex config");
        let input =
            generate_email_with_regex_inputs(&args.from_domain, &raw_email, &regex_info, None)
                .await
                .expect("Failed to generate email with regex inputs");
        stdin.write(&input);
    } else {
        let raw_email = read_email_file(&args.email_path).expect("Failed to read email file");
        let input = generate_email_inputs(&args.from_domain, &raw_email, None)
            .await
